...
export default function Sidebar({ open, onToggle, activeTab }: SidebarProps) {
  ...
  return (
    <aside
      className={`w-64 md:w-64 bg-slate-900 border-r border-slate-700 transition-all duration-300 flex flex-col overflow-hidden md:relative ${
        open ? "relative" : ""
      }`}
    >
      {/* Logo/Brand Area */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-slate-700 bg-slate-800">
        <div className="text-2xl font-bold text-blue-400">Scanner Pro</div>
      ...
      <nav className="flex-1 overflow-y-auto p-3">
        {sections.map((section) => (
          <div key={section} className="mb-6">
            <p className="text-xs font-semibold text-blue-300 uppercase px-4 py-2 tracking-wider">{section}</p>
                ...
                return (
                  <div key={item.id}>
                    {item.blocked ? (
                      <button
                        disabled
                        className="w-full flex items-center gap-3 px-4 py-3 rounded-md text-slate-600 opacity-50 cursor-not-allowed mb-2 transition-all text-sm md:text-sm"
                        title={t.featureUnavailable}
                      >
                        <div className="relative flex-shrink-0">
                          <Icon className="w-5 h-5" />
                          <Lock className="w-3 h-3 absolute -bottom-1 -right-1 bg-slate-800 rounded-full p-0.5" />
                        </div>
      ...
      {/* Footer */}
      <div className="border-t border-slate-700 p-3 text-center text-xs text-slate-400 bg-slate-800">
